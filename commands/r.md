...
- `r` 単体入力 → 同上
- `/r NNN` → 番号指定で単発取り込み（NNN は 2〜4 桁の数字、メニュー省略）
- `r NNN` → 同上
...
if not reply2 or reply2.get("verb") == "ng":
    # 処理中フラグが残っていれば（同一セッション内で他処理から start していれば）クリアを試みる
    try:
        _http_post(f"/{items[0]['code']}/cancel-processing")
    except Exception:
        pass
    print("⏹️ 承認なし → 中断")
    sys.exit(0)

# 処理中フラグを立てて PWA に視覚化（オレンジ chip + ⚙️ 処理中バッジ）。
# consume / archive で自動クリアされるが、Step 5 中断時のために PWA 側で手動キャンセルも可能。
try:
    _http_post(f"/{items[0]['code']}/start-processing")
except Exception as exc:
    print(f"⚠️ start-processing 失敗（処理は続行）: {exc}")
