...
- **振り分けフィルタが無言で止まることがある**: Thunderbirdの自動振り分けフィルタが
  原因不明のまま機能しなくなり、アカウント自体は正常に同期していても専用フォルダ
  だけ9日間1通も増えなかった実例(2026-09-18)。`extract_new.py`が「新着0件」を
  返しても鵜呑みにせず、対象mboxのmtimeを一度は疑うこと。対処は「0.5」節参照
