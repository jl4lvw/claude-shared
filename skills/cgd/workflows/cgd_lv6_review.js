...
// NOTE: only the *input path* lost its default. label / codex_reasoning / include_gemini
// still fall back to defaults on purpose — those cannot silently review the wrong thing.
// NOTE: Lv6 takes no aux_input_path (all three reviewers get the same input). That
// asymmetry against Lv7/Lv8 is intentional, not a missing check.
const _missing = []
...
if (_missing.length > 0) {
  return {
    ...
    message: 'Required args are missing: ' + _missing.join(', ')
      ...
      + ' / correct form: args { input_path, label } (codex_reasoning / wf_nonce are optional)'
      + ' / NOTE: codex_file and aux_file are NOT valid key names.',
  }
}
const inputPath = _args.input_path
// 綴り違い('High' 等)をそのまま渡すと codex 側の挙動と timeout 判定
// (=== 'high') が食い違い、high 相当の処理が 300 秒で切られる。
const _rawReasoning = String(_args.codex_reasoning || 'medium').toLowerCase()
const reasoning = ['low', 'medium', 'high'].indexOf(_rawReasoning) !== -1 ? _rawReasoning : 'medium'
...
//
// なお **Lv6 は aux_input_path を取らない**（3 者に同一入力を渡す構成）。
// aux が必須なのは Lv7/Lv8 だけで、この非対称は仕様である。
...
const pre = await agent(
  `cgd Lv6 の入力を検証します。**レビューはしません。ファイルの中身を解釈・要約しないでください。**
...
2. 標準出力の全文を Write tool で C:/tmp-ai/cgd_raw_${r.name}_${label}_${_runTag}.md に保存する (後で人が生ログを確認できるように)。
...
if (authFailed.length > 0) {
  log(`[review] 認証エラー検出: ${authFailed.join(', ')} → Lv6 中断 (参加者${reviewers.length}者揃わないと意味がない)`)
  return {
    halt: 'auth_error',
    reviewers_source: reviewersSource,
    failed: authFailed,
...
if (ok.length < reviewers.length) {
  log(`[review] レビュアー欠員: ${ok.length}/${reviewers.length} のみ成功 → Lv6 中断`)
  return {
    halt: 'incomplete',
    reviewers_source: reviewersSource,
    got: ok.length,
...
// 列名は SKILL.md の統合表と揃える。自前で先頭大文字化すると
// deepseek が「Deepseek」になり、lv7/lv8 の「DS」と食い違う。
const displayNames = { codex: 'Codex', gemini: 'Gemini', deepseek: 'DS', qwen: 'Qwen' }
