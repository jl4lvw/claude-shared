...
let reviewers = [
  ...
  }] : []),
  {
    ...
    cmd: `python "${CLAUDE_DIR}/tools/deepseek_coder.py" --role reviewer "__INPUT_1__"`,
    timeout: 600000, usage: true, isCodex: false,
    authSignals: 'AuthenticationError / 401 / invalid api key / DEEPSEEK_API_KEY が設定されていません',
  },
  {
    name: 'qwen', kind: 'tech',
    cmd: `python "${CLAUDE_DIR}/tools/qwen_advisor.py" --role reviewer "__INPUT_1__"`,
    timeout: 600000, usage: true, isCodex: false,
  ...
  },
  {
    ...
    cmd: `python "${CLAUDE_DIR}/tools/deepseek_coder.py" --role critic "__INPUT_1__"`,
    timeout: 600000, usage: true, isCodex: false,
    authSignals: 'AuthenticationError / 401 / invalid api key / DEEPSEEK_API_KEY が設定されていません',
  },
]
...
if (codexOk < 3) {
  ...
  return { reviewers_source: reviewersSource, halt: 'codex_incomplete', got: codexOk, message: 'Lv8 は Codex medium(技術) / high(技術) / high(批評) の3本すべての成功が必須です。' }
}
if (ok.length < reviewers.length) {
  log(`[review] レビュアー欠員: ${ok.length}/${reviewers.length} のみ成功 → Lv8 中断`)
  return { reviewers_source: reviewersSource, halt: 'incomplete', got: ok.length, expected: reviewers.length, message: `Lv8 は参加者${reviewers.length}者全員の成功が必要です。` }
}
