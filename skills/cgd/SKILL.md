...
- **Serena MCP は任意**（Lv7/Lv8 の関連関数抽出を高精度化する。`claude mcp list` で `serena` が `Connected` なら自動優先、未登録のプロジェクトでは自動的に grep+Read へフォールバックする。導入必須ではない）
  - **2026-08-27夜〜: サブプロジェクト単位スコープ化**（モノリポ全体を毎回インデックスすると1インスタンス1GB超になることが実測で判明したため）。Serenaは起動時に特定プロジェクトを持たず、セッション側が明示的に選ぶ方式に変更した:
    - 読み取り専用の探索（`find_symbol`/`find_referencing_symbols`等）→ 共有 ProjectServer（port 24225、`SerenaProjectServer`タスクで常駐）経由の `mcp__serena__query_project` を優先。どのセッションから問い合わせても実体は1個分のインデックスで済む
    - 共有サーバーが未起動、または実際にコードを編集する（`replace_symbol_body`等）場合 → `mcp__serena__activate_project` でそのセッションに必要なサブプロジェクトのディレクトリだけをローカルに持つ（約200MB/件、モノリポ全体より大幅に軽い）
    - ユーザーが会話中で「◯◯をSerenaに追加」と言ったら `activate_project(<そのサブプロジェクトの絶対パス>)` を呼ぶ
    - 「今Serenaにつながっているサブプロジェクト一覧を出して」→ `python C:/ClaudeCode/.claude/tools/project_server_launcher.py --status`（共有サーバーに今ロードされているものを表示。システムのpythonでよい）
    - 「切り離して」→ `python C:/ClaudeCode/.claude/tools/project_server_launcher.py --unload <name_or_path>`
    - 詳細・設計判断の経緯は `.claude/tools/project_server_launcher.py` と `.claude/tools/serena_launcher.py` のdocstring参照
...
**直前指示にレベル明示があればスキップ**:
- 「Lv0」「委譲」「scaffold」「量産」「Codex に実装させる」「Codex で書かせる」「テスト量産」「docstring 一括」「コスト節約」 → **Lv0**（Codex が実装）。**手順は `/lv0` スキル**（この節の続きではなく `/lv0` を呼ぶ）
...
> **Lv1 / Lv5 / Lv6 はこの明示指示でのみ選ばれる**（自動選択の表からは外してある・後述）。
> トリガ語は残してあるので、ユーザーが名指しすれば従来どおり動く。
...
| 関数間の整合性・状態管理・広範囲の変更・大規模モジュール内のクロスリファレンスが論点 | **Lv7** |
| 技術の最深掘りと使い勝手の批評を同時にやる必要がある | **Lv8** |

**Lv7-8 を自動で選ぶときの歯止め**: 主 context とサブスククォータの消費が大きいので、**宣言に「なぜこの重さが要るか」を必ず添える**。理由が 1 行で書けないなら Lv2〜4 に落とす。

#### Lv1 / Lv5 / Lv6 は自動選択の対象から外してある（2026-08-13・実測に基づく）

**廃止ではない。** ユーザーが「Lv6 で」と明示すれば従来どおり動く。
自動選択の表から外したのは、**この 3 つが隣のレベルに構造的に支配されていて、
合理的に選ぶと決して選ばれない**ため。実測 85 件（2026-07-27〜08-13）で
Lv1 / Lv5 / Lv6 はいずれも **0 件**だった。

| Lv | 元の選択基準 | 支配している隣 |
|---|---|---|
| Lv1 | 小さな差分・低リスク・軽い確認 | **Lv2**（「迷ったらこれ」が既定なので、軽い側は常に Lv2 で足りる） |
| Lv5 | リリース直前・🔴 は自動で潰したい | **Lv6/7/8** がいずれも 🔴 自動修正 1 周を含む |
| Lv6 | 広範囲の変更で盲点が怖い・複眼で潰したい | **Lv7**（Codex 2 本 + 補助へ専用抜粋で全軸が上位互換。コスト差もわずか） |

Lv6 を選びたくなる状況は**必ず Lv7 を選ぶ理由にもなる**ので、表に残しておくと
「選べるように見えて選ばれない」選択肢が判断を重くするだけになる。
そこで Lv6 の基準（広範囲の変更）は **Lv7 の行に統合した**。

> **0 件を「不要」と読み替えていない。** pv の SKILL.md が警告しているとおり、
> 使用回数だけでは「需要が無い」と「選択ロジックが選ばない」を区別できない。
> ここで判断の根拠にしたのは件数ではなく**支配関係の構造**で、件数はその裏付けである。
> Lv0 が 0 件なのは別の理由（委譲レーンという直交軸を使っていないだけ）なので、
> 表に残してある。

**Lv8 は「Lv7 + 批評 2 名」である。** `cgd_reviewers.build_reviewers()` の出力を比べると、
Lv7 の 4 者は Lv8 の技術枠と `cmd` / `timeout` まで完全に一致する。
これは重複ではなく Lv8 の定義そのもので、実装とドキュメントは一致している。
**Lv7 と Lv8 を分ける唯一の軸は critic の有無**、と読んでよい。
...
**見える一覧は CLI の版で変わる。** 2026-09-08、A 端末は codex-cli 0.147.0 では
`gpt-6-astra` が一覧に出ず、0.153.4 へ更新して初めて現れた(TK 端末では見えていた)。
**端末間で版がずれていると「見えているモデルが違う」状態になり、切り分けが難しくなる**
ため、A / TK とも 0.153.4 に揃えた(2026-09-08 運用者判断)。
...
- ⚠️ **このコマンドの出力は必ず読む（2026-08-28 変更・旧版の「無視してよい」は撤回）**。
  Lv6/7/8 では**ゲートを張るかどうかがここで決まる**ので、失敗を無視すると
  「張れたつもりで WF を回す」か「気づかず全セッションを止める」のどちらかになる。
  **非 0 で終わったら Step 2 へ進まないこと**（ゲートが張られていない）
- **セッション単位で張る（既定）**。`--session` を省略しても環境変数
  （`CLAUDE_CODE_SESSION_ID`）から自セッションを解決する。解決できないときは
  **張らずに非 0 で終わる**（fail closed）。全セッションを止めたい場合だけ `--global` を付ける。
  旧版は省略時に**全セッションを遮断する全体ゲート**を張っており、
  2026-08-27 に別セッションの WF を `gate_ambiguous` で停止させた
- ⚠️ **Lv6 / Lv7 では副作用が1つある**: このコマンドが `.claude/hooks/cgd_wf_gate.py arm` を呼び、**inline の `codex exec` を遮断するゲート**を張る（`[cgd wf-gate] Lv7: WF 必須ゲートを張りました` が stderr に出る）。以後 `codex exec` を Bash で直接叩くと PreToolUse hook が deny する。**Lv6/Lv7 は Workflow で実行すること**（後述「Workflow 経由実行」）。ゲートは WF 起動を検知すると自動解除され、保険として 180 分で失効する。**`disarm` は自分のゲートと所有者不明の全体ゲートだけを解除する**（他セッション所有のものは消さず、誰の持ち物かを表示して非 0 で終わる。2026-08-27 に他セッションのゲートを実際に破壊したため 2026-08-28 に変更）
...
## Lv0: 委譲レーン（Codex に実装を任せる）→ **手順は `/lv0` スキル**

> **2026-09-19 分離**: Lv0 の手順は軽量スキル **`/lv0`**（`.claude/skills/lv0/SKILL.md`）に移した。
> Claude の作業は「仕様を書く → 承認 → `cgd_lv0_auto.py run` を 1 回起動 → レポートを読む」だけで、
> 準備・Codex 実行・機械検査（node / pytest / ruff / Playwright）・失敗時の Codex への自動出し直しはドライバが回す。
> **Lv0 が選ばれたら、この節ではなく `/lv0` を呼ぶ**（cgd 全文は約 76K トークンあり、Lv0 では読み込む価値が無い）。
> 旧手順（Claude が prepare / run / diff / 検証を 1 つずつ叩く）は [reference/lv0_manual_steps.md](reference/lv0_manual_steps.md)、
> DeepSeek／Qwen に書かせる代替手順は [reference/lv0_ds_qwen_fallback.md](reference/lv0_ds_qwen_fallback.md)。

Lv1-8 が「**レビュー強度**を上げる」のに対し、Lv0 は「**実装の主体**を Claude → Codex に切り替える」コストレーン（品質レーンとは直交）。
番号の慣例とは違い「軽量」ではなく例外モードで、**1 回の委譲が約 200 行未満なら Claude が直接直す方が安い**（2026-09-19 の実測。損益分岐は約 200 行）。

**前提（満たさないと動かない）**:
- サンドボックス補助 exe のパスが 260 文字未満の Codex CLI が要る（`C:\tools\codex-cli`。`cgd_lv0_codex.py resolve` が判定。INC-20260918-155756c98ac7）
- Codex のサンドボックスは **ユーザープロファイル配下の Python / Node を起動できない**ので、テストは Codex ではなく**機械（ドライバ）が回し**、失敗は Codex へ戻す
- サンドボックスは作業フォルダと %TEMP% 以外に書けず、ネットワーク無し
- 検査の Playwright / Chromium は導入済み（Python 版）。`--review deepseek` を使うときだけ `DEEPSEEK_API_KEY` も要る

**ドライバの入口**: `python .claude/tools/cgd_lv0_auto.py {plan,run,check}`（`--help` 参照）。エンジンは `cgd_lv0_codex.py`（prepare / run / diff / restore）。
...
**抽出方法（優先順・Serenaが使えるプロジェクトかどうかで分岐）**:

**A. Serena が利用可能な場合（優先）**: `mcp__serena__*` ツールがロード済み、かつ対象ファイルの言語に対応する language server が有効なとき。**Serenaは起動時に特定プロジェクトを持たない**ため、以下は読み取り専用ツール（`find_symbol`/`find_referencing_symbols`等）に限り共有 ProjectServer 経由（`query_project`）に寄せる。共有サーバーが未起動なら `activate_project(<対象サブプロジェクトの絶対パス>)` でそのサブプロジェクトだけをローカルに持ってから同じ手順を行う:
1. `git diff` で変更 hunk のあるシンボル（関数/メソッド）を特定
2. 変更された各シンボルに `find_symbol`(`include_body=true`) — grep の境界誤検出（宣言行の見落とし・別関数の巻き込み）が起きない
3. **`find_referencing_symbols` で実際の呼び出し元を取得** — ここが Lv7 の狙い（Integration バグ＝関数間の暗黙の前提違反）に直結する。grep は同名文字列に一致するだけで「本当に呼んでいるか」を判定できないが、Serena は構文木上の実参照のみを返すため、**関連関数の見落とし・誤検出そのものが減る**（トークン削減はその副産物）
4. 取得した本体＋参照元スニペットを `C:/tmp-ai/lv7_related_funcs.txt` に結合保存

**B. Serena が未登録／対象言語が未対応の場合（フォールバック・旧手順）**:
...
#### run metrics — Lv6/7/8 の効果を後から測れるようにする（2026-08-13 追加）

pv Lv3 で 4 者が「**Codex を medium + high で多重化する効果を一度も測っていないし、
測る仕組みも無い**」で収束したことへの対応。ただし「記録を毎回叩く」を新しい手順に
すると、cgd の usage log と同じで急いでいるときに飛ぶ。そこで層を分けてある。

| 層 | 何を持つか | 誰が記録するか |
|---|---|---|
| プロセス層 | どのレビュアーが走り、`.exit` と生ログのサイズがどうだったか | **`collect` が自動で書く**（新しい手順は増えない） |
| findings 層 | 指摘の出所（med/high/両方/補助のみ）と severity の内訳 | `record-merge` に WF の戻り値を渡す（任意） |
| 採用層 | どの指摘を実際に直したか | **まだ記録していない** |

findings 層まで残すなら、統合表を描いた後に WF の戻り値を **標準入力**で渡す
（引数に載せると引用とサイズで壊れる）:

```bash
python "C:/ClaudeCode/.claude/tools/cgd_plan.py" record-merge --run <PLAN_RUN> < "C:/tmp-ai/wf_result.json"
```

集計はいつでも取れる:

```bash
python "C:/ClaudeCode/.claude/tools/cgd_plan.py" metrics
```

**出るのは「挙がった」件数であって「採用された」件数ではない。**
採用層が無いので、この表だけで Codex 多重化の費用対効果を結論づけてはいけない
（出力にも毎回その断りが出る）。`source` が付かない指摘も `?` 列として必ず数える
——合計から落とすと割合が実態より良く見えるため。

##### location の内訳も同時に取る

`record-merge` は findings の `location`（`file:line`）を**入力に含まれる差分に
照らして**分類し、`metrics.json` の `locations` に残す。差分パースは
`pv_review.py` の純関数を再利用している（56 件のテスト付き）。

| 分類 | 意味 |
|---|---|
| `in_diff` | 差分に在るファイルで、行が hunk の範囲内 |
| `out_of_diff` | 差分に在るファイルだが hunk の外（**呼び出し元への言及など。正当**） |
| `file_not_in_diff` | 差分に無いファイル。**cgd では異常ではない**（Codex は周辺を読める） |
| `no_line` / `unparsed` | ファイル名だけ / location が無い・解釈できない |

**これはゲートではない。** cgd の Codex は read-only sandbox で周辺ファイルを
読める設計なので、差分外の指摘を pv のように幻覚と断定できない。合否は変えず、
判断材料を増やすだけ。ただし `file_not_in_diff` が大半を占めるなら
「対象を外れたレビューになっていないか」を疑う手がかりにはなる。

**入力に差分が無ければ `checked: false` を返し、件数は一切出さない。**
照合していないのに 0 件と出すと「問題なし」と誤読されるため。
...
#### ⚠️ 失敗した WF を `resumeFromRunId` で再開しない（2026-08-12）

`resume` は `(prompt, opts)` が同じ agent の結果を**再生**する。ゲート状態を取ってくる
Preflight agent もそのひとつなので、**前回実行時の nonce** がコマンドへ差し込まれる。
ゲートを張り直した後だと必ず照合に外れ、全レビュアーが deny されて
`halt: exec_failed` になる。**args も script も同じである以上、WF 側では防げない。**

2026-08-12 に、修正が正しく入っているのに resume したせいで再び全 deny になり、
「修正が効いていない」と誤診した（`INC-20260812-172431c53929`）。

- 失敗した run をやり直すときは **`resumeFromRunId` を外して新規実行**する
- どうしても resume したいなら `args.wf_nonce` に現在の値を明示する
  （`cgd_wf_gate.py nonce`）
- 症状で見分けられる: nonce が古いときだけ、deny メッセージが
  **「このコマンドには nonce が付いていますが、現在のゲートの値と違います」**と名指しする
...
| Lv7: 関連関数の事前抽出（Serena優先／フォールバックgrep+Read） | 認証エラー/欠員チェック（全員成功で次段） |
