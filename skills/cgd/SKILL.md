...
| Gemini | 長文解析・調査・案出し・2枚目レビュー（Lv7 では補助役）。**旧 gemini CLI は廃止 → `gemini_advisor.py`（OpenAI互換API・要 `GEMINI_API_KEY`）に移行** | `python "<絶対パス>/gemini_advisor.py" --role <reviewer\|advisor> "<file>"` |
...
- **Gemini は旧 gemini CLI（無料tier）が 2026/06 に廃止された**ため、`gemini_advisor.py`（Google の OpenAI 互換エンドポイントを叩く Python ラッパー）を使う。環境変数 `GEMINI_API_KEY`（Google AI Studio で無料発行）が必要。既定モデル `gemini-2.5-flash`（`GEMINI_MODEL` で上書き可）
- Codex は `OPENAI_API_KEY` をセットしない（ChatGPT サブスク認証）。Gemini は `GEMINI_API_KEY` を使う（AI Studio 無料枠中心で実費は極小）
...
# Bash #2（Gemini）— プロンプトを先にファイル化してパス渡し（日本語の CP932 化け回避・argv 安全）
#   cat > "C:/tmp-ai/gemini_input.txt" <<'EOF' ... <Gemini プロンプト> ... EOF を先に書く
python "C:/ClaudeCode/.claude/tools/gemini_advisor.py" --role reviewer "C:/tmp-ai/gemini_input.txt"
```

`cd "C:/tmp-ai"` は日本語 CWD 文字化け回避。Codex は `< /dev/null` でハング防止（必須）。
Gemini は `gemini_advisor.py`（OpenAI互換API）に移行済み。**プロンプトは必ずファイルに書いてパス渡し**（Windows の CP932 で日本語 argv/stdin が壊れるため）。要 `GEMINI_API_KEY`。
...
Lv1-3 でも Codex の呼出回数は記録する（サブスククォータ消費量の把握目的）。
Lv2 以降は Gemini（API 従量・極小）、Lv4-5 / Lv6 は DS / Qwen の従量課金もあるので必須中の必須。
...
| Gemini | N 回 | 計 X tok | 計 Y tok | ¥G（従量・極小） |
...
- **Gemini / DS / Qwen の入力・出力・費用**: stderr の `[Gemini Usage] 今回:` / `[DS Usage] 今回:` / `[Qwen Usage] 今回:` の数値を **複数回呼んだ場合は積算**（Gemini も API 移行で従量課金化。AI Studio 無料枠中心なので通常 ¥0〜数十銭）
...
Codex CLI / Gemini API / DeepSeek API / DashScope (Qwen) API の **いずれか一つでも認証エラーを返したら、そこで即中断**する。残り段を片肺で続行しない・自動で別ツールに切り替えない。

**検出シグナル（例）**:
- Codex: `Not logged in` / `401` / `unauthorized`
- Gemini: `openai.AuthenticationError` / `401` / `API key not valid` / `GEMINI_API_KEY が設定されていません`
...
1. どの API で何のエラーが出たか
2. 復旧手順（`codex login` / `GEMINI_API_KEY` の設定確認 / `DEEPSEEK_API_KEY` の設定確認 / `DASHSCOPE_API_KEY` の設定確認）
3. 復旧後に `/cgd` を再実行する旨
...
- **相談段は read-only**: Codex `--sandbox read-only`、Gemini/DS/Qwen は API 単発呼び出し（書き込み権限なし）
- **書き込みフェーズは Step A のみ**: ファイル編集・新規作成は **Step A に集約**。Codex / Gemini / DS / Qwen には絶対に書き込ませない
- **API キー**: Gemini は `GEMINI_API_KEY`、DS は `DEEPSEEK_API_KEY`、Qwen は `DASHSCOPE_API_KEY` を読む。Codex のみサブスク認証（`OPENAI_API_KEY` をセットしない）
