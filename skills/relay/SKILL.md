...
### 返事が数時間後に来る委託の型(2026-08-24)

上の例は「その場で数分待つ」短い往復。**作業一式を委ねて数時間後に返事をもらう**
場合は既定のままだと足りない。TTL 15分では待っている間に切れ、切れた瞬間に
常駐GUIがそのスレッドを拾う。

```
/relay send --to RC --reserve --reserve-ttl 32400 "作業一式をお願いします"   # 9時間
/relay wait --thread <TID> --holder cli:xxxxxxxx --after-id <MID> --keep
```

- `--reserve-ttl` の上限は **32400秒(9時間)**。`handoff` の予約と同じ長さ
- `--keep` … 待ち切れても**予約を返さない**。既定は返す(短い往復ではそれが正しい)
- 待ち続けたいときは `wait --keep` を繰り返す。呼ぶたびにTTLが延びる

**無期限にはできない。** セッションはコンテキスト圧縮・Ctrl-C・スリープで予告なく
消えるので、上限のない予約は「誰も見ないメッセージ」を生む。

**守ること**:

- **処理が終わったら必ず `release` する。** 予約はTTL付き(既定15分・最長9時間)なので
  放置してもいずれGUIに戻るが、その間そのスレッドは誰の目にも留まらない
- `wait` がタイムアウトすると**自動で予約を解放**し、終了コード3で戻る。以降はGUIが引き取る
  (`--keep` を付けたときは解放せず、残り時間を表示して終了コード3で戻る)
- 予約中のスレッドは `check`(holder無し)には出てこない。**出ないものは自分の担当ではない**
- `send` は毎回「返信の受け取り手」を1行で申告する。**`--reserve` を付け忘れると
  そこに「常駐GUI」と出る。** 出たら、その場の案内どおり `reserve` で掛け直す
- GUIのメッセージツリーには予約中スレッドが「🔒 セッション対応中 残N分」と表示される。
  隠しきらないのは、対応中なのか落ちて放置されているのかを運用者が見分けられるようにするため
- 予約が切れると **GUI がログに「期限切れになりました」と出してから引き取る**。
  黙って引き取ることはない
- 運用者はスマホ(PWA の設定タブ「セッションが対応中」)から**予約を強制解除できる**。
  解除されると holder 側は次の延長で予約を失う。TTL は保険で、こちらが逃げ道
...
1. 転送先に、**元の本文ごと**入った1通が同じスレッドに届く
2. スレッドの相手が自分 → 転送先に差し替わる。以後 **転送先は自分の名義のまま原送信者と直接ラリーできる**
3. 自分の未処理と予約が外れる(担当から抜ける)
4. スレッドは履歴として残り、GUIのツリーでは**取り消し線 + 「〜へ引き継ぎ済」**で表示される

**代理(delegation)との違い**: 代理は「相手の名義で振る舞う」もので、原送信者からは元の担当者に見える。
引き継ぎは「担当そのものを渡す」もので、**転送先の名義が表に出る**。継続的に担当が移るなら引き継ぎを使う。

**制約(意図的)**:
- 自分宛のメッセージだけ転送できる(送信者は転送できない)
- スレッドの相手本人へは転送できない(それは転送ではなく返信)
- **一度引き継ぎに関わった相手へは差し戻せない**(往復のたらい回し防止)。上限は1スレッド5回
- 自動転送ルールは**作っていない**。転送するかは受信した側がその場で判断する
...
### `/relay answer` — 未回答の確認に答える(2026-08-21)

確認(ask)は常駐GUIのAIが判断に迷ったとき運用者へ出す質問で、**受信箱とは別枠**。
しかも発端メッセージが `done` になると `GET /messages` から消えるため、
以前は `check` が「未処理メッセージはありません」と言うのに未回答が残る、
という食い違いが起きていた(本番 msg#918)。いまは `check` の末尾に
「■ 未回答の確認が N 件あります」として必ず出る(0件なら何も出ない)。

```bash
python "C:/ClaudeCode/.claude/skills/relay/scripts/relay_client.py" answer <ask_key> "回答の本文"
```

- `ask_key` は `check` の表示にそのまま出ている
- 回答の置き場と渡し方はスマホから答えたときと同じ(常駐GUIが取りに来て、待っている処理へ渡す)。
  **どこから答えたかで挙動が分かれないようにしてある**
- **危険操作の承認(approval)・隔離解除(quarantine)は答えられない**(403)。
  スマホでは PIN を要求している種類なので、CLI だけ素通しにはしない。パソコンの承認パネルで行う
- 二重に答えると 409「もう回答が届いています。パソコン側の反映を待っています」
- 「発端メッセージは完了済み」と注記が付いたものは取り残されたゴミ。答えなくてよい
  (常駐GUI 1.45.0 以降が 60 秒周期で自動的に閉じる)
## エラー時
