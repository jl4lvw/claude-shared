...
### Step 3: 画像調達ルール(2026-09-09確定・必須)

**main画像とother画像で調達経路を分ける**(ユーザー確定ルール):

- **main画像は必ず何らかの加工(背景純白化・構図調整・不要テキスト除去等)が要る前提**
  なので、楽天連携の候補グリッドから選ばせない。**ユーザーにファイルパスまたはURLを
  貼ってもらう**(NAS共有パス `\\192.168.1.50\...` 形式が主。まれにWeb URLのこともある)。
  受け取ったら都度、解像度(長辺1000px以上)と背景純白率を実測してから採用可否を報告する。
- **other画像(最大6枚)は楽天連携の候補グリッド(A, B, C…)からクリックで選ばせる**。
  下記「クリック選択式アーティファクト」を必ず使う(テキストで「A, C, E」と申告させる
  旧方式は使わない)。
