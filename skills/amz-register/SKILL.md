...
| 項目 | 備考 |
|---|---|
| Amazon価格 | Eストア価格とは別に設定されることが多い。DBの`price`と`estore_price`のどちらとも異なる場合がある。**同シリーズを続けて登録する場合でも毎回確認する**(前件と同額を無断で流用しない)。**ワッペンはサイズ帯別の標準価格ルールあり**(2026-09-11確定): 4×6cm=¥1,280、直径・幅10cm前後の丸/楕円=¥1,870。該当サイズなら初期値として提案してよいが、確定はユーザー確認後(詳細は`project_amz_wappen_pricing_rules`メモリ) |
