...
同一受注内の数量合計が**4個まではゆうパケ、5個以上は佐川急便**に自動で切り替わる
(`ARARE_YUPACKET_MAX_QTY = 4`。2026-09-14以前は`ARARE_CLICKPOST_MAX_QTY`という名前で
クリックポストだった。ユーザー指示でゆうパケへ変更・GoQの実選択肢名は「ゆうパケット」
ではなく「ゆうパケ」)。ただし上記の「合算しない」ルールにより、通常は
