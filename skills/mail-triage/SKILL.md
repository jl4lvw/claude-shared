...
<!-- SKILL_VERSION: 2026-09-08_214755 -->
...
**`Sent` フォルダだけを見てはいけない。** 053.ai-agent は SMTP で直接送るため
Thunderbird の Sent を経由せず、控えが `003.AI-agent` や `fw-terashita/INBOX` に
自分宛コピーとして届く。2026-09-08、これを見落として**しのびや.com への 9/4 の返信を
「未返信・期限超過」と誤って報告した**（ユーザーの指摘で発覚）。

対象は `accounts.py` の `SENDING_IDENTITIES` に持たせてある。
**ここに足すのは「意図して書いた返信を送る名義」だけにすること。** WEBショップの
自動返信を出す `shopmaster@` を入れると、注文確認メールで顧客の問い合わせが
「返信済み」になって黙って閉じる（`tests/test_mail_index.py` で固定済み）。
