...
- `/handoff save 名前` の引数があれば、`save` より後ろ全体をその名前として採用する。ただし下記の要件を満たさなければ整えてから使う
- 引数が無ければ、Claude が「作業のゴール」から命名し、**1 行で宣言**してから保存する。ユーザーには聞かない（Claude が自動決定・ユーザーの言い直しを優先）
- 要件（＝「問題のない名前」の基準。load 時の検証にも同じものを使う）:
  - 1 行であること（改行・タブ・その他の制御文字を含まない）
  - 空でないこと
  - 長さは全角 30 文字前後を目安とし、**上限は 40 文字**（超えたら意味を保って短縮する）
...
### Step 4: 次セッション名を決める
「次セッション名のルール」節に従って名前を 1 つ決める。

- `/handoff save 名前` の引数があれば、それを採用する（要件を満たさなければ整える）
- 引数が無ければ、「作業のゴール」から Claude が命名し、**1 行で宣言**する（例: `次セッション名は「丹青社売上連携 工程3〜6」にします`）
- 決めた名前を、次の Step 5 のテンプレートにある `次セッション名:` 行へ入れる
...
2. **名前を検証する**（「次セッション名のルール」の要件で再確認）。問題があれば、handoff の「作業のゴール」から **適切な名前を作り直して**それを使ってよい。問題の例:
   - 空、または改行・制御文字を含む
   - 40 文字を超える
