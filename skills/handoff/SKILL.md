---
name: handoff
description: セッション引継ぎスキル。コンテキストが重くなったときに現在の作業状態を作業フォルダ直下の `.handoff/` へMarkdown保存し、次セッションで復元できるようにする。`/handoff save` で保存（3桁ID発行）、`/handoff load [NNN]` で読込、`/handoff list` で一覧。
...
### Step 5: 保存完了メッセージ
保存パスと **発番した 3 桁 ID** を表示し、次セッションで `/handoff load` で最新読込／`/handoff load NNN` で当該 handoff をピンポイント復元できることを伝える。
...
次セッションでは:
  /handoff load        ← 最新を読込
  /handoff load 427    ← この handoff をピンポイント読込
```
