...
### Step 3: スキル/コマンドの重複を確認（読み取り専用）

`/g-dl` が配るのは project の `.claude/` だけで、**ユーザー階層 `~/.claude/skills`（このPC全体用）には
触れない**。この PC に古い複製が残っていると、ユーザー階層が優先されて **いま取り込んだ修正が
新しいセッションに届かない**（2026-09-19 の handoff の事故）。取り込み後に必ず確認する:

```bash
python "C:/ClaudeCode/.claude/tools/skill_scope.py" check
```

- `exit 0` → 重複なし
- `exit 1` → 表示された `retire` コマンドの実行をユーザーに勧める（**自動では実行しない**。
  退避先は削除ではなく `~/.claude/_retired_skills/`）

### Step 4: 完了報告

取り込んだコミット数（あれば）、ミラー成否、hook 登録結果（変更なし / N 件登録＋要再起動）、
スキル重複の有無を 1〜2 行で報告。
