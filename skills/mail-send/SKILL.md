...
| **返信メールを引用なし・自己流の本文だけで作る** | 返信は必ず `/mail-reply` の「本文→区切り線→引用元」形式に従う（後述 Step 0） |
| アーティファクトに生成日時を書かない | 見出し直下に「YYYY-MM-DD HH:MM JST時点」を必ず入れる（後述） |
...
0️⃣  返信・転送なら、まず元メールの本文を /mail-search で取得する
    【返信の場合】本文を組み立てる前に必ず /mail-reply を呼び、
                「本文→区切り線→引用元」の引用形式を適用する(下記参照)
    【転送の場合】転送の定型フォーマットを使う(後述)
...
**🔴 2026-09-24の事故**: 顧客(柴山様)への発送状況の返信で、CLAUDE.mdの「`/mail-send`を
呼ぶ」だけに従い`/mail-reply`を呼ばず、引用なしの自己流本文でアーティファクトを作成・
承認・EML生成まで進めてしまった。`/mail-reply`は末尾の関連リンクとしてしか
参照されておらず、「返信なら分岐して踏む」という導線が本文中に無かったのが原因。
**返信メールは、本文を書く前に必ず`/mail-reply`のStep 1️⃣（引用形式）を適用する。**
...
### 🔴 返信の場合: 本文を書く前に必ず `/mail-reply` を呼ぶ

「返信して」の依頼は`/mail-send`だけでは完結しない。**返信本文には元メールの引用が
必須**で、その形式（本文→区切り線→引用元。引用が上に来る形式は禁止）は`/mail-reply`
に定義されている。`/mail-send`の中に簡略化した例文を書いて済ませず、返信を作る
ときは`Skill(mail-reply)`を呼んでその形式に従うこと。

2026-09-24、顧客からの発送状況問い合わせへの返信で、`/mail-reply`を呼ばずに
引用なしの本文を作ってしまいアーティファクト承認まで進んだ実例がある
（本文取得はできていたが、引用形式への変換ステップが抜けた）。
...
### 🔴 生成日時を必ず表示する(2026-09-18の指摘)

**「アーティファクトが古くなって放置される」こと自体は完全には防げない**
(ユーザーが後日そのタブを開き直すことは普通にある)。防ぐべきなのは
**「いつ時点の内容か分からないまま読まれること」**の方。だから対策は
「放置しない」ではなく「**見た瞬間にいつの内容か分かる**」形にすること。

- 生成・更新の**都度**、見出し直下に生成日時(JST、分単位まで)を入れる
  (例: `2026-09-17 19:03 JST時点`)
- **アーティファクトを再利用・上書き更新するときも、この日時を必ず更新する**
  (前回の日時のまま中身だけ差し替えない)
- 実例(2026-09-18): 前日(9/17)に生成した「送信予定メール」アーティファクトを
  ユーザーが翌日開き直し、「10時間放置されている」という古い注意書きを見て
  「これは今の話か」と確認する手間が発生した。生成日時さえ見えていれば
  一目で「昨日の、既に処理済みの内容」と分かったはずだった

### HTMLアーティファクトの最小構成

```html
<h1>送信予定メール(まだ送信していません)</h1>
<div class="sub">2026-09-17 19:03 JST時点の内容</div>
<div class="card">
...
いません」という見出しと、その直下の生成日時を必ず入れる**(下書き済みなのか送信済みなのか、
いつ時点の内容なのか、ユーザーが一目で分かるようにするため)。

### チェックポイント(送信前に必ず確認)

- [ ] 生成日時(JST)が見出し直下に表示されているか
- [ ] 宛先・Ccは正しいか
- [ ] 敬語・表現は適切か
...
**2026-09-18 更新（生成日時の明記）**: 052卸売の産経デジタル向け報告メールで、前日
(9/17)作成の「送信予定メール」アーティファクトを翌日ユーザーが開き直し、古い注意書き
（「10時間放置」）を見て「これは今の話か」と確認する手間が発生した。ユーザー指摘
「アーティファクトが放置されることよりも、作られた時刻が見える形で明示されていないことが
問題」を受け、生成日時(JST)を見出し直下に必ず表示するルールを追加した。
