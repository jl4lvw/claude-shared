...
#
# **これはゲートではない。** 2026-08時点ではcgdのCodexはread-only sandboxで周辺ファイルを
# 読める設計だったため、「差分に無いファイルへの指摘」はpvと違って正当でありうる、
# という前提でここは合否を変えない設計にした。
# 2026-09-11: Codex CLI v0.154.0でexec_command(ファイルオープン)がblocked by policyに
# なり、Codexは実際には周辺ファイルを読めなくなった(INC-20260911-123532bde1cc)。
# それでも「幻覚と断定できないものを弾くと正当な指摘を誤判定で捨てる」という
# 判断自体は変わらない(関連関数抜粋の事前同梱で似た情報は渡っているため、
# file_not_in_diffの指摘が全て無根拠というわけではない)。ゲート化するなら
# このコメントごと設計を見直すこと。
